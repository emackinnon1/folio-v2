...
it was unsurprising that another company [snapped them
up](https://tidbyt.com/blogs/tidbyt/tidbyt-is-joining-modal?srsltid=AfmBOorQMUiwm4YpVtTMcSK7aZSDvUyKm0K4U_AHwvBODOPYDgzb2Xbo).
...
You'll need to use the `fileName` param along with whatever Starlink params are
needed for a particular app. Here is an example of generating the aurora applet
...
When you include the param `fileName=digital_rain` in the URL, Axilla looks for
the file at `functions/axilla/assets/digital_rain/digital_rain.star`.
...
[writeup](https://dev.to/bogdaaamn/run-your-nodejs-application-on-a-headless-raspberry-pi-4jnn)
for more detailed information, but you'll want to open up the firewall settings
to allow traffic on port 3000 and be sure `pm2` is installed globally.

You should have node already installed from setting up pixlet, but now you will
...
In the tutorial linked at the beginning of this section, the author shows you
how add a script that will start this app when the Pi boots up.
...
navigate to the app using a browser. Use your hostname that you set up way back
...
":3000" to the end of the hostname and route traffic to the Pi more efficiently,
but for now I have ufw enabling ingress traffic. I'll probably have to reverse
the steps I took to set up ufw, but that is a future worry.
...
over at Tidbyt), so you'll have to excuse any ugly code. Like the axilla fork, I
only changed the things that I need to get this up and running.

You'll need the Platform IO VS Code extension installed and a usb-c cord to
connect to your machine. The repo's
...
<Img
  mdx
  publicId='/images/projects/tidbyt-diy/platform-io-tidbyt.png'
  alt='Platform IO menu'
  width={700}
  height={600}
...
use to attach the case to the matrix with.
