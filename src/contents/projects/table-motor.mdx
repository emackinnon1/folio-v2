...
EDIT (Jan 18, 2025):
I have added a switch to my setup to ensure that I can still use this table motor when the internet is down, meaning it won't get locked shut. The updated code is located in the [project on github](https://github.com/emackinnon1/pico-table-motor/tree/main).
I have opted for a switch but you could use a simple stateless "click" button with a resister, leveraging the code to manipulate state and drive the motor.
...
Countersink bolts (the countersunk nut fit the linear actuator through hole perfectly; also used these to attach the bottom brackets of the actuator to the table):
