...
With simple logical rules, these tortoises displayed behavior, to a degree, that single cell organisms might display, but even single cell organisms seem to have rudimentary
abilities to "learn by experience" patterns. Grey Walter set out to create a new version that could do the same. His next iteration ported a sensor that could detect
whistles and mixed that signal with the signals from the bump sensor and the light sensor. This new circuit allowed the
tortoise to be "trained" to come to the whistle as if it were a light. The humble, plain circuit of Walter's tortoises seemed to serve as the platform for rudimentary
Pavlovian learning, the famous pairing of an unrelated stimulus to a behavior. What the mixing of the signals allowed for is a basic logical _or_ statement (with _W_ representing
the whistle signal, _L_ representing the presence of a light signal and _B_ representing the subsquent behavior of heading toward the stimulus):
...
## Facades of intelligence in the unicellular world
...
In the same way that Walter's second iteration of tortoises combined logical operations (`if whistle present, then light attraction behavior` or `battery low and light present -> 
...
As molecule _A_ snaps into place and binds to a site on enzyme _X_ specially suited for it, _X_ transforms into a slightly different enzyme with a minutely different
...
binding site anymore, so it breaks off. This is an allosterica enzyme in action. Inhibition _or_ disinhibition (like our example here) can be achieved, dictating the
logical:
